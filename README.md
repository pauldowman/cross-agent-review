...
The prompt tells reviewers to make no changes, but a prompt is not a security boundary. The harness's configured sandbox or permission profile is responsible for enforcing access. OpenCode's plan agent denies its dedicated edit tool but still allows shell commands, so its actual protections depend on the surrounding configuration. OpenCode runs with `--format json`, and the tool extracts the last completed text event as the review. omp runs with `--mode json --no-session`, so a reviewer's run stays out of your own omp history, and the tool takes the text of the last assistant message that carries any, since a run cut short can end on a tool call it never answered. That keeps whatever the reviewer had written rather than discarding it; whether the text is a finished review is settled when its grade is parsed.
